    ...
    function pquery($Query_String,$debug=false){
        try {
            ...
            for($i=0;$i<count($this->param);$i++){
                $cpar = $this->param[$i];
                if($curparam[_length]) $this->Stmt->bindParam($cpar[_key],$cpar[_val],PDO::PARAM_STR,$cpar[_size]);
                else if($curparam[_datatype]) $this->Stmt->bindParam($cpar[_key],$cpar[_val],$curparam[_datatype]);
                else {
